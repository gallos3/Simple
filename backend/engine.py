...
from graph_rag import search_graph_corpus  # GraphRAG replaces legal_rag
from database import run_queries_batch
...
from procedure_map import PROCEDURE_ALIASES as PROCEDURE_MAP_ALIASES
...
def _execute_followup() -> str:
    ...
    from llm_interface import generate_cypher_query
    ...
    from llm_interface import answer_general_question
...
def agent_answer(question: str, previous_question: str = "", from_voice: bool = False, role: str = None, web_search_enabled: bool = True) -> Union[str, Dict[str, Any]]:
    ...
    from config import ROLE_PERMISSIONS, DEFAULT_ROLE, ROLE_TRAINEE
    from cag_cache import get_cag_cache
    ...
    if intent == "procurement_simulation":
        from procurement_simulation import build_simulation_prompt, SIMULATION_SYSTEM_PROMPT
    ...
    if intent == "procurement_report_card":
        from procurement_simulation import generate_report_card
        import json
    ...
    if intent == "mixed_legal_data":
        ...
        from agentic_loop import create_agent
        from cag_cache import get_cag_cache
...
def disambiguate_authority_by_nuts(entity: dict, question: str) -> dict:
    ...
    try:
        from database import get_authority_nuts_name
    except Exception:
        return entity
...
def _summarize_graph_elements(elements: list, authority_name: str = None, year: str = None) -> str:
    ...
    """
    from entity_extractor import format_authority_name
...
def _execute_matched_query(
    ...
    if "$procedure" in cypher:
        from procedure_map import PROCEDURE_ALIASES as PROCEDURE_MAP_ALIASES
        from entity_extractor import detect_procedure_type
    ...
    from query_matcher import detect_output_type
    ...
    # ----------------------------------------------------
    from llm_interface import summarize_query_result
...
def _maybe_attach_followup(
    ...
    try:
        from llm_interface import generate_followup_question
