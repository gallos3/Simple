...
def tool_calculate_vcd(params: str) -> str:
    ...
    lines.append("  💡 Επεξήγηση: Το VCD εξετάζει αν οι ανάδοχοι που κερδίζουν πολλές συμβάσεις είναι και αυτοί που απορροφούν τη μεγαλύτερη αξία. Υψηλό VCD δείχνει ασυμμετρία μεταξύ πλήθους συμβάσεων και οικονομικής αξίας.")
...
def tool_calculate_entropy(params: str) -> str:
    ...
    lines.append("  💡 Επεξήγηση: Η εντροπία μετράει πόσο ομοιόμορφα κατανέμονται οι συμβάσεις. Τιμές κοντά στο 0 δείχνουν συγκέντρωση σε λίγους φορείς ή αναδόχους, ενώ υψηλότερες τιμές δείχνουν μεγαλύτερη διασπορά.")
...
def tool_calculate_ici(authority: str) -> str:
    ...
    lines.append("  💡 Επεξήγηση: Ο δείκτης ICI συνδυάζει τη συγκέντρωση της αγοράς με την επαναληψιμότητα των συνεργασιών. Υψηλή τιμή σημαίνει πιο κλειστό δίκτυο με επαναλαμβανόμενους αναδόχους και αυξημένη ανάγκη ελεγκτικής διερεύνησης.")
