...
# STREAMING ENDPOINT
# =============================================================================
from core.streaming_endpoint import add_streaming_routes
...
def ask():
    ...
    from core.engine import answer_question  # Safe re-import (already loaded)
...
def upload_file():
    ...
    if ext == "csv":
        ...
        try:
            from data_access.database import ingest_csv_to_neo4j
    ...
    if ext == "pdf":
        ...
        try:
            ...
            from utils.legal_ingest import ingest_pdf_to_corpus
            ingest_pdf_to_corpus(save_path)
            
            # 2. Για άμεση χρήση στη μνήμη (Live RAG)
            from rag.legal_rag import ingest_pdf
...
def export_docx():
    from core.engine import generate_full_audit_report
...
def export_summary():
    from core.engine import run_illegal_award_checks_and_export_docx
    year = request.args.get("year", "2024")
...
def download_latest_export():
    from core.engine import get_latest_export_path
...
def submit_feedback():
    ...
    """
    from utils.debug_logger import save_feedback_report, get_last_trace
    data = request.get_json(force=True) or {}
...
from analytics.audit_store import AUDIT_CASES
