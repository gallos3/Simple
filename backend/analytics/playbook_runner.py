...
from data_access.entity_extractor import load_entity_cache, extract_entity_from_question, extract_year_from_question
from analytics.audit_store import create_audit_case, get_case
from analytics.audit_runner import run_audit_S1_to_S4
...
def run_illegal_direct_awards_playbook(question: str, from_voice: bool = False) -> str:
    ...
    """
    from data_access.entity_extractor import load_entity_cache, extract_entity_smart
    ...
    if not entity:
        clear_pending_illegal_awards()
        return (
            "Για να τρέξω τον **Έλεγχο ενδείξεων κατατμήσεως / υπέρβασης ορίου (S1–S4)**, "
            "γράψε την **πλήρη επωνυμία** της αναθέτουσας αρχής και το **έτος**.\n"
            "Παράδειγμα: «Έλεγξε για ενδείξεις κατάτμησης το ΠΓΝΘ ΑΧΕΠΑ για το 2024»."
        )
...
def format_illegal_awards_summary(case: Dict[str, Any]) -> str:
    ...
    return (
        f"**Έλεγχος ενδείξεων κατατμήσεως / υπέρβασης ορίου (S1–S4)**\n"
        f"Αναθέτουσα: **{authority}**\n"
        f"Έτος: **{year}**\n\n"
        f"- **S1** Ενδείξεις κατακερματισμού αναθέσεων: **{s1}**\n"
        f"- **S2** Συγκέντρωση αναθέσεων ανά CPV: **{s2}**\n"
        f"- **S3** Συγκέντρωση αναθέσεων ανά CPV-5: **{s3}**\n"
        f"- **S4** Συγκέντρωση αναθέσεων σε ίδιους αναδόχους: **{s4}**\n\n"
        f"Αν θέλεις λεπτομέρειες, πες: **S1**, **S2**, **S3** ή **S4**.\n\n"
        f"⚠️ Τα αποτελέσματα αποτελούν στατιστικές ενδείξεις και όχι οριστική αξιολόγηση νομιμότητας. Απαιτείται περαιτέρω ελεγκτική διερεύνηση."
    )
